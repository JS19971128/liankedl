	...
	<view class="bank_card_list">
	   <view class="china_bank" v-for="(item,index) in list" :class="{'bg_color': oddEven(index) == false}" >
	   	  <view class="list-touch clearfix" @touchstart="touchS" @touchmove="touchM" @touchend="touchE" :data-index="index" :style="item.txtStyle" @click="pickBank(item)">
	   	  	...
	   	  	<image class="card_logo" src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/common/card_logo.png" mode=""></image>
	   	  </view>
		  
		  <view class="list-delete" @click= "deleteMember(item.id,item)" >
			  删除
		  </view>
	   </view>
                        ...
                        if (res.confirm) {
                            _this.$fly.post(`/transfer/deleteBankById?id=` + id)
                            	.then(res => {
                            		...
                            		uni.hideLoading();
                            		if (res.code == 0) {
										console.log(item,_this.$store.state.bankInfo)
										if(item.createDate == _this.$store.state.bankInfo.createDate){
											_this.$store.state.bankInfo=''
										}
										console.log(item,_this.$store.state.bankInfo)
                            			uni.showToast({
                            			    title: '删除成功！',
