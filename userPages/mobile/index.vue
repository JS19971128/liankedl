	...
	<view class="container">
		<view class="top fz-14">
			<view class="flex_center item">
				...
				<input type="text" v-model="mobile" focus="true" placeholder="请输入真实有效的手机号" placeholder-style="font-size:28rpx;line-height:28rpx;color:#BCBCBC"/>
			</view>
		</view>
		 <!-- v-if="!username" -->
		...
		<!-- <button class="btn item flex_center fz-14" open-type="getUserInfo" @getuserinfo="getWxUserInfo">开启我的链客智慧商圈</button> -->
		 <!-- v-if="!username" -->
		<view class="btn flex_center fz-14" @click="checking(save)">{{save}}</view>
	</view>
</template>
...
<script>
	export default{
		...
		methods:{
			...
			async checking(btnText){
				console.log(this.$store.state.userInfo)
				if(this.username&&btnText=='更改手机号'){
