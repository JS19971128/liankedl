		...
		<view class="keyboard flex_center">
			...
			<view class="right fz-16">
				<view class="delete flex_center" @click="deleteNum">
					<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/order/delete.png" mode="widthFix"></image>
				</view>
...
<script>
	export default{
		...
		onLoad:function(query){
			...
			// 微信登录
			this.query = query;
			// const q = decodeURIComponent(query.q).split('?')[1].split('=')[1]
			// this.$store.commit('SCANCODEFUN',q);
		},
		onShow:function() {
			let query = this.query;
			
			console.log(decodeURIComponent(query.q),'queryId')
			let merchantId = decodeURIComponent(query.q).split('=')[1]
