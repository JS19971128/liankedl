<template>
	<view class="bg">
			...
			<scroll-view scroll-y="true" style="height:500px" @scrolltolower="getMoreCoupon">
				<view v-for="(item,i) in list" :key="item.id">
					...
					<view class="item flex_center" v-if="item.couponShowType=='IMAGE'&&item.couponType=='MERCHANT'" @click="chooseCoupon(i)">
						<view class="brand">
							<image class="brand-bg" src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/coupon/juxing.png"></image>							
						...
						<image class="rightDefe" src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/coupon/right.png" mode="widthFix"></image>
						<view class="choose" v-show="chooseList[i]['choosed']"><image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/coupon/choosed.png" mode="widthFix"></image></view>
					</view>
					<!-- 普通优惠券 -->
		...
		<view class="no_data" v-else>
			<view class="">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/coupon/coupon_none.png" mode="widthFix"></image>
			</view>
...
<script>
	export default{
		...
		methods:{
			...
			payment(){  //支付
				// 未拿到用户信息时提示用户再点击一次
				console.log("支付", this.$store.state)
				...
				// params.pricePaid= params.pricePaid/100
				// params.priceTotal=params.priceTotal/100
				// #ifdef MP-WEIXIN
				params.payMode = 'wx_lite';
				// #endif
				// #ifdef MP-ALIPAY
				// params.bizType = 'AppPayApplet';
				// params.payType = 'APPLET';
				params.payMode = 'alipay_lite';
				...
				console.log(params)
				try{
					this.$fly.post('/adapay/pay',params)
					.then(res=>{
						console.log(res,'原始')
						...
						if(res.code === 0){
							...
							let url = JSON.parse(res.data.expend.pay_info).tradeNO;
							console.log()
							/*if(url){
			...
			aliPay(tradeNo){ //调起支付宝支付
				uni.requestPayment({
					...
					complete: (com)=>{
						console.log("com", com, tradeNo)
					}
				})
			},
			wechatPay(payInfo) {  //微信支付
			console.log('执行',payInfo)
				uni.requestPayment({
					appId: payInfo.appId,
...
<style lang="scss" scoped>
	...
	.main{
		...
		.item{
			...
			margin: 0 auto;
			margin-top: 20rpx;
			background: url('https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/coupon/bg.png') no-repeat;
