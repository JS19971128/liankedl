<template>
	<view class="raiders_zone">
		...
		<view class="event_goods">
			<view class="event_goods-box" :class="{'box-bottom':showBottom==0}" @tap="chooseMe(0,'pending')">
			...
			<view class="event_goods-box" :class="{'box-bottom':showBottom==1}" @tap="chooseMe(1,'success')">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/hot.png" mode="widthFix"></image>
		...
		<view class="treasure_bdi" v-if="list.length > 0">

			<view class="wrap_list_content" v-for="(item,index) in list" :key='index' @click="goProductDetails(item.id)">
...
<script>
	...
	export default {
		...
		methods: {
			...
			navListFun() {
				this.$fly.get(
					`/app/draw/home/list?size=10&page=${this.page}&userId=${this.userId}&sort=createDate,desc&deleteFlag=false`).then(
					res => {
						let data = res.data;
