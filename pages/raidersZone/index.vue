<template>
	<view class="raiders_zone">
		<view class="raiders_bg">
			<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/tu-min.png" mode="widthFix"></image>
			<view class="voucher">
			...
			<view class="treasure" @click="voucherRule=true">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/rule.png" mode="widthFix"></image><text
				 class="text_rule">夺宝规则</text>
			</view>
		</view>
		<!-- 夺宝规则 -->
		<view class="wrap_flex_bg" v-if="voucherRule">
			<view class="rule_bdi">
				<image class="shut_down" @click="voucherRule=false" src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/border_close.png"
				 mode="widthFix"></image>
		...
		<view class="consumption" v-if="consumptionRule">
			<view class="middle_content">
				<image class="shut_down" @click="consumptionRule=false" src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/border_close.png"
				 mode="widthFix"></image>
		...
		<view class="event_goods">
			<view class="event_goods-box" :class="{'box-bottom':showBottom==0}" @tap="chooseMe(0,'success')">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/hot.png" mode="widthFix"></image>
				<text class="active_text">已开奖</text>
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/hot.png" mode="widthFix"></image>
			</view>
			<view class="event_goods-box" :class="{'box-bottom':showBottom==1}" @tap="chooseMe(1,'pending')">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/hot.png" mode="widthFix"></image>
				<text class="active_text">夺宝中</text>
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/hot.png" mode="widthFix"></image>
			</view>
			<view class="event_goods-box" :class="{'box-bottom':showBottom==2}" @tap="chooseMe(2,'gameOver')">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/hot.png" mode="widthFix"></image>
				<text class="active_text">已结束</text>
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/hot.png" mode="widthFix"></image>
			</view>
		</view>
		<!-- 列表 -->
			...
			<view class="wrap_list_content" v-for="(item,index) in navList" :key='index' @click="goProductDetails(item.id)">
				...
				<view class="right_title_schedule">
					...
					<view class="treasure_icon" v-if="item.drawPercent < 1" @click.native.stop="openConsumptionRule(item)">
						<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/treasure.png" mode="aspectFill"></image>
					</view>
		...
		<uni-load-more v-else :iconSize="20" color="#999999" :status="status" :contentText="contentText"></uni-load-more>
		<!-- <view class="no_data" v-else>
			<view class="wrap_image_src">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/underConstruction.png" mode="widthFix"></image>
			</view>
			<view class="tip">
	...
	export default {
		...
		methods: {
			...
			startGrabTreasure() {
				...
				this.$fly.get(`/app/draw/addDraw?userId=` + this.$store.state.userInfo.id + '&drawCommodityId=' +
						participateTreasureHunt.id + '&couponCount=' + this.voucher)
					.then(res => {
						uni.hideLoading();
						if (res.code == 0) {
			...
			navListFun(status) {
				console.log(status, this.drawStatus)
				this.$fly.get(
					`/app/draw/home/list?size=10&drawStatus=${this.drawStatus}&page=${this.page}&userId=${this.userId}&sort=createDate,desc&deleteFlag=false`).then(
					res => {
						let data = res.data;
						...
						// 	if (item.drawStatus == this.drawStatus) {
						// 		// console.log(this.navList)
						// 		this.navList = this.navList.concat(item);
						// 	}
						// 	// if (item.drawStatus == this.drawStatus) {
						// 	// 	this.navList = this.navList.concat(item)
						// 	// }
			...
			chooseMe(index, status) { //夺宝栏目选择
				if (this.showBottom == index) {
					return
				} else {
					this.showBottom = index
				}
			...
			getList(choose) { //获取列表
				...
				this.$fly.get(
						`/app/draw/myDrawList?userId=${this.$store.state.userInfo.id}&page=${this.page}&size=20&drawStatus=${this.drawStatus}&sort=createDate,asc&deleteFlag=false`
					)
					.then(res => {
						console.log(res)
						uni.hideLoading();
