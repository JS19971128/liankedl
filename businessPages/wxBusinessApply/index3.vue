	...
	<view class="container">
		...
		<view class="info fz-14">
			...
			<view class="main">
				...
				<view class="item flex_center" v-if="bank.zhlxLabel === '对公账户'">
					<view class="item-name">开户证明</view>
					<view class="item-content flex_center fz-12">
						<view class="upload flex_center" v-for="(item,index) in imgArr" :key="index">
							<!-- 正面 -->
							<view class="uploadPic flex_center" v-if="item.credentialUrl" @click="upload(index,'credentialUrl')">
		...
		<view class="btn fz-14 flex_center" @click="next(false)">下一步</view>
		<w-picker
			class="address"
			mode="region" 
			:visible.sync="show" 
...
<script>
	import {code as codeNum} from '@/common/util/bank_number.js'
	import {cityDataCode as city_code} from '@/common/util/adapay_bank_city_code.js'
	export default{
		data() {
			return {
				zhlx:[{label:'对公账户',type:'1'},{label:'个人账户',type:'2'}],
				zhlxLabel:'',
				zhlxIndex:0,
		...
		methods:{ 
			onConfirm($event){ //省市区选择
				...
				// this.bank.baneCityCode = $event.value[1]
				console.log(this.bank,$event)
				console.log(city_code.length,$event.obj.city.label)
				for(let i = 0;i < city_code.length;i++){
					for(let b = 0;b<city_code[i].cities.length;b++){
						if($event.obj.city.label.includes(city_code[i].cities[b].label)){
							// console.log(city_code[i].cities[b].label)
							console.log(city_code[i].cities[b].value)
							this.bank.baneCityCode = city_code[i].cities[b].value
							return false
						}
					}
				}
