	...
	<view class="container">
		...
		<view class="" v-if="status==1">
			<view class="main flex_center">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/business/review.png" mode="widthFix"></image>
				<view class="tip">
		...
		<view class="" v-else>
			<view class="main flex_center">
				<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/business/unpass.png" mode="widthFix"></image>
				<view class="tip">
	...
	export default{
		...
		methods:{
			goApply(){
				...
				uni.redirectTo({
					url:'/businessPages/wxBusinessApply/index?shopCenter=true'
				})
			},
			// 上传图片
			upImg(index,type){
								...
								if(res.size < 5000){
									...
									uni.uploadFile({
										...
										success:res=>{
											...
											this.$fly.post('/adaEnter/uploadEnterScreen'+`?userId=${this.userId}&imgURL=${url}`).then(res=>{
												uni.showToast({
													title:res.message,
													icon:'none'
												})
