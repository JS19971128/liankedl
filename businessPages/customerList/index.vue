<template>
	<view class="container">
		...
		<view class="main" v-if="list.length>0">
			<view class="item flex_between" v-for="item in list" :key="item.id">
				<view class="item-left flex_center">
					<!-- 支付宝头像 -->
					<!-- #ifdef MP-ALIPAY -->
					<view><image :src="item.aliHeadImg || 'https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/my/avatar.png'" mode="widthFix"></image></view>
					<!-- #endif -->
					<!-- 微信头像 -->
					<!-- #ifdef MP-WEIXIN -->
		...
		<view class="no_data" v-else>
			<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/my/friend_none.png" mode="widthFix"></image>
			<view class="tip">
