...
<script>
	export default{
		...
		methods:{
			...
			async payment(){//支付
				try{
					...
					if(this.ifziying){
						data.merchantId = 71434
					}
					console.log(data)
					// #ifdef MP-WEIXIN
					data.payMode = 'wx_lite';
					// #endif
					// #ifdef MP-ALIPAY
					// data.bizType = 'AppPayApplet';
					// data.payType = 'APPLET';
					data.payMode = 'alipay_lite';
					...
					let res = await this.$fly.post('/adapay/prePay',data);
					console.log(res)
		...
		onLoad:function(query){
			this.init()
			this.getCoupon(this.changeNeedPay);
			console.log(query,'query')
			if(query.ziyingurl=='1'){
				this.ifziying = 1
			}
		}
	}
