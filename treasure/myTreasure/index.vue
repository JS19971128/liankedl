	...
	<view class="my_treasure">
			...
			<view class="treasure_list" v-for="(item,index) in list">
				<view class="image_title_status" @click="goProductDetails(item.id)">
					...
					<view class="right_info_name">
						...
						<view class="treasure_status"><text class="active_status" :style="{color:returnOrderStatus(item.drawStatus).color}">{{returnOrderStatus(item.drawStatus).status}}</text><image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/common/more_gray.png" mode="aspectFill"></image></view>
					</view>
				</view>
				<!-- 继续夺宝--确认订单 -->
		...
		<view class="consumption" v-if="consumptionRule">
			<view class="middle_content">
				<image class="shut_down" @click="consumptionRule=false" src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/shop/border_close.png" mode="aspectFill"></image>
...
<script>
	export default {
		...
		methods: {
			...
			tabItem(status) {
				if(this.drawStatus==status) return
