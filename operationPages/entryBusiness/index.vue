<template>
	<view class="container">
		...
		<scroll-view scroll-y="true" class="list" @scrolltolower="reachBottom">
			<view class="item flex_between" v-for="item in list" :key="item.id" @click="goCheck(item)">
				<view class="info flex_center">
					...
					<view class="shop">
						<view class="fz-14">{{item.companyName}}</view>
		...
		<!-- <view class="no_data flex_center" v-else>
			<image src="https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/operation/merchant_none.png" mode="widthFix"></image>
			<view class="tip">
	...
	export default{
		data() {
			return {
				...
				tab:[
					{id:1,text:'全部',value:''},
					{id:2,text:'已通过',value:'1'},
					{id:3,text:'已驳回',value:'9'},
					{id:4,text:'待审核',value:'0'},
					{id:5,text:'已完成',value:'1'},
				],
				page:0,
				list:[],
				total:0,
		...
		methods:{
			...
			getList(){ //获取进件商家列表
				...
				this.$fly.post(`/adapayEnter/queryMerchantEntryList?${formateObjToParamStr(data)}`)
				.then(res=>{
					...
					if(res.code == 0){
						...
						data.forEach(item=>{
							item.createDate = formatTime(item.createTime)
							item.businessLicenseUrl = JSON.parse(item.enterPicParam).storePic
						})
