		...
		methods:{
			...
			async updateEntryStatus(){
				try{
					...
					let data = await this.$fly.post(`/adapayEnter/updateEntryStatus?id=${id}&status=FINISH`);
					uni.showToast({
					    title: '修改成功！',
					    duration: 2000,
						icon:'none'
			...
			async init(){
				try{
					...
					let details = await this.$fly.post(`/adapayEnter/findMerchantEntryByUserId?${formateObjToParamStr(data)}`);
					...
					// 	INTERIOR_PHOTO:'室内照'
					// }
					// for(let i of form.enterPicParam){
...
<style lang="scss" scoped>
	.container{
		...
		.top{
			...
			&.AUDITED{
				background-image: url(https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/operation/sh_01.png);
			}
			&.PASS{
				background-image: url(https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/operation/sh_02.png);
			}
			&.FINISH{
				background-image: url(https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/operation/sh_03.png);
			}
			&.FAILED{
				background-image: url(https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/operation/sh_04.png);
			}
			.top-left{
				width: 530rpx;
				line-height: 50rpx;
		...
		.tab{
			...
			.active{
				...
				&::after{
					...
					width: 20rpx;
					height: 10rpx;
					background: url(https://lianketong.oss-cn-shenzhen.aliyuncs.com/wx_images/home/tab_icon.png) no-repeat;
